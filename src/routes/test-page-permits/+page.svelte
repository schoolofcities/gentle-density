<script>
    ...
    import MapV from "../lib/MapVictoriaSecondaryLaneway.svelte";
    import MapC from "../lib/MapCalgarySecondaryLaneway.svelte";
	import MapE from "../lib/MapEdmontonSecondaryLaneway.svelte";
	import MapT from "../lib/MapTorontoSecondaryLaneway.svelte";
    ...
    let activeMap = 'MapV'; // Set the default active map

  	// Function to switch the active map
...
<main>
	...
	<div class="background-white">
		...
		<div class="text">
			...
			<p>
				Swaths of Toronto consists of low-density single family homes (these lands are often called the <a href="https://schoolofcities.github.io/yellowbelt-canadian-cities-2022/">Yellowbelt</a> due to the historical exclusionary zoning of these neighbourhoods). 
			...
			<p>
				Around 2018-2019, the City of Toronto started taking accelerating actions to increase missing middle housing supply, in what is now the <a href="https://www.toronto.ca/city-government/planning-development/planning-studies-initiatives/expanding-housing-options/">Expanding Housing Options</a> program. 
	...
	<div id="top-bar">
        <div class="buttons">
            <button class:selected={activeMap === 'MapV'} on:click={() => switchMap('MapV')}>City of Victoria</button>
            <button class:selected={activeMap === 'MapE'} on:click={() => switchMap('MapE')}>City of Edmonton</button>
            <button class:selected={activeMap === 'MapC'} on:click={() => switchMap('MapC')}>City of Calgary</button>
            <button class:selected={activeMap === 'MapT'} on:click={() => switchMap('MapT')}>City of Toronto</button>
        </div>
    </div>

    <!-- Rendering the maps based on the activeMap -->
	{#if activeMap === 'MapV'}
		<MapV/>
	{:else if activeMap === 'MapE'}
		<MapE/>
	{:else if activeMap === 'MapC'}
		<MapC/>
	{:else if activeMap === 'MapT'}
		<MapT/>
	{/if}
