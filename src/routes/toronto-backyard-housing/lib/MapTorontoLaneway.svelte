<script>
	...
	function filterResYellow() {
		...
		onIncome = false;
		if (onResYellow) {
			map.setPaintProperty('zoneAllow', 'fill-opacity', 0);
			onResYellow = false
		} else {
			map.setPaintProperty('zoneAllow', 'fill-opacity', 1);
			onResYellow = true
		}
	}
...
<div id="top-bar">
	<p>Location of laneway and garden suites in the City of Toronto:</p>
</div>
...
<div id="options-wrapper">
	...
	<div id="options">
		...
		<div id="pointLayers">
			...
			<div id="rearYardButton" on:click={filterRearYard} class="{onRearYard ? 'layerOn' : 'layerOff'}" >
				<svg width=20 height=10>
					<circle
						style="fill:#8DBF2E;stroke-width:2;stroke:#fff"
						cx="15"
						cy="5"
						r="4" />
				</svg>
		...
		<RangeSlider bind:values range pips all='label' step={1} min={2018} max={2025} hoverable={false}/>
		...
		<div id="pointLayers">
			...
			<div id="resYellowButton" on:click={filterResYellow} class="{onResYellow ? 'layerOn' : 'layerOff'}" >
				...
				Permissible development areas <br> (based on zoning by-law)
			</div>
