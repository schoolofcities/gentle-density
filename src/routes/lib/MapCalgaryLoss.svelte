<script>
	...
	import transitLines from '../assets/calgary/transitLines_calgary.geo.json';
	import transitStops from '../assets/calgary/transitStops_calgary.geo.json';
	...
	let map;
	let PMTILES_URL = "/gentle-density/calgary.pmtiles";
	...
	onMount(() => {
		...
		map.on('load', function() {
			...
			map.addSource('calgaryBoundary', {
				'type': 'geojson',
				'data': calgaryBoundary
			});
			map.addLayer({
				'id': 'calgaryBoundary',
				'type': 'line',
				'source': 'calgaryBoundary',
				'layout': {},
				'paint': {
					'line-color': '#fff',
					'line-width': 1,
